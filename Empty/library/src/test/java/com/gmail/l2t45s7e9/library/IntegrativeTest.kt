...
import com.gmail.l2t45s7e9.java.interactor.*
...
fun Root.setUpMocks(): Suite.() -> ContactDetailsViewModel {
    ...
    return {
        ...
        val notificationInteractor: NotificationInteractor = BirthDateNotificationModel(dateModel, notificationRepository)
        val contactDetailsInteractor: ContactDetailsInteractor = ContactDetilsModel(contactDetailsRepository)
...
private fun Suite.mockDefaults(contact: Contact) {
    ...
    beforeEachTest {
        coEvery { contactDetailsRepository.loadDetailsInformation(contact.id, contact.contactColor) } returns flow { emit(contact) }
    }
