...
private const val VERTICAL_SPACE_ARGUMENT: Int = 8
...
class ContactListFragment : Fragment(R.layout.contact_list_fragment) {
    ...
    private val contactItemDecorator: ContactItemDecorator by lazy(LazyThreadSafetyMode.NONE) {
        ContactItemDecorator(
            (VERTICAL_SPACE_ARGUMENT * resources.displayMetrics.density).toInt()
        )
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        contactListViewModel.listLiveData.observe(
            viewLifecycleOwner,
            { result: List<Contact?> ->
